# python draw-Central-LF.py --task SR_mnist --graph Centralized_n=10_b=0 --partition LabelSeperation --portrait
...
args.task = 'SR_mnist'
args.graph = 'Centralized_n=10_b=1'
# args.partition = 'DirichletPartition_alpha=1'
...
pic_name = 'centralized_' + task_name + '_' + graph_name + '_' + method + '_' + partition_name + '_' + attackNames[0][0]
...
if args.portrait:
    fig, ax_matrix = plt.subplots(len(attackNames))
    if len(attackNames) == 1:
        ax_matrix = [ax_matrix]
    for i, (attack_code_name, _) in enumerate(attackNames):
        # ax_obj['loss'][attack_code_name] = ax_matrix[i][0]
        ax_obj['accuracy'][attack_code_name] = ax_matrix[i]
        
    ax_matrix[-1].set_xlabel(r'iteration k', fontsize=FONT_SIZE)
    ...
    fig, ax_matrix = plt.subplots(1, len(attackNames))
    if len(attackNames) == 1:
        ax_matrix = [
...
for attack_code_name, attack_show_name in attackNames:
    for agg_index, (agg_code_name, agg_show_name) in enumerate(aggregations):
        ...
        # file_name = 'CSGD' + '_' + attack_code_name + '_' + agg_code_name \
        #     + '_invSqrtLR' + mark_on_title
        # file_name = 'CSGD' + '_' + attack_code_name + '_' + agg_code_name \
        # + '_ladderLR' + mark_on_title
        file_name = method + '_' + attack_code_name + '_' + agg_code_name \
        + mark_on_title
...
if args.portrait:
    ...
    ax_matrix[-1].legend(loc='lower left', bbox_to_anchor=(1.1,0.1),
            borderaxespad=0., ncol=1, fontsize=FONT_SIZE)
else:
    ax_matrix[-1].legend(loc='lower left', bbox_to_anchor=(0.1,-0.4),
                           borderaxespad=0., ncol=1, fontsize=FONT_SIZE)
# fig.tight_layout()
