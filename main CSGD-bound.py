...
from ByrdLab.centraliedAlgorithm import CSGD, CSGD_under_DPA, CSGD_under_DPA_compute_bound
...
# data_package = mnist()
# task = softmaxRegressionTask(data_package, batch_size=6000)

# data_package = fashionmnist()
# task = softmaxRegressionTask(data_package)

# data_package = cifar10()
# task = NeuralNetworkTask(data_package, batch_size=5000)

data_package = mnist()
task = NeuralNetworkTask(data_package, batch_size=6000)
...
if 'label' in attack_name:
    env = CSGD_under_DPA_compute_bound(aggregation=aggregation, honest_nodes=honest_nodes, byzantine_nodes=byzantine_nodes, attack=attack, step_agg = step_agg,
...
log('[Start Running]')
_, loss_path, acc_path, Bound_A_list = env.run()
...
if record_in_file:
    ...
    dump_file_in_cache(title + '_Bound_A_full_batch', Bound_A_list, path_list=path_list)
print('-------------------------------------------')
