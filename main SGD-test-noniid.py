...
from ByrdLab.library.cache_io import dump_file_in_cache, dump_model_in_cache, load_file_in_cache
from ByrdLab.library.dataset import ijcnn, mnist, fashionmnist, cifar10, covtype, mnist_sorted_by_labels
...
# ALFA
path_list = [task.name, graph.name, 'LabelSeperation'] + workspace
q = load_file_in_cache('q-end', path_list=path_list)
ratio = 0.1
flipped_data_size = int(ratio * len(task.data_package.train_set))
num_classes = task.data_package.num_classes
assert len(q) == flipped_data_size * num_classes
...
for i in range(len(q)):
    if q[i] == 1:
        ...
        if index != i:
            print('The index of q:', i)
            print('The index of training dataset:', index)
            print('The label of the flipped data:', task.data_package.train_set.targets[index])
