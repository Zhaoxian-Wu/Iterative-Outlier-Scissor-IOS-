...
from ByrdLab.library.dataset import ijcnn, mnist, fashionmnist, cifar10, covtype
...
# ALFA
path_list = [task.name, graph.name, 'TrivalPartition'] + workspace
q = load_file_in_cache('q-end', path_list=path_list)
data_size = len(task.data_package.train_set)
num_classes = task.data_package.num_classes
assert len(q) == data_size * num_classes
